	...
	private void deserializeData(MessageAndMetadata<byte[], byte[]> consumerRecord, List<Record> results){
		...
		if (value instanceof List) {
			...
			for (int i = 0; i < rs.size(); i++) {
				Object v = rs.get(i);
				if (v instanceof Map) {
					results.add(new KafkaMapRecord(asynESOutPutDataTran.getTaskContext(),key, (Map<String, Object>) v,consumerRecord.offset()));
				} else {
			...
			results.add( new KafkaMapRecord(asynESOutPutDataTran.getTaskContext(),key, (Map<String, Object>) value,consumerRecord.offset()));
		} else if (value instanceof String) {
			results.add(new KafkaStringRecord(asynESOutPutDataTran.getTaskContext(),key, (String) value,consumerRecord.offset()));
		}
		else{
			if(logger.isWarnEnabled()){
				logger.warn("unknown value type:{}",value.getClass().getName());
			}
			results.add(new KafkaStringRecord(asynESOutPutDataTran.getTaskContext(),key, String.valueOf( value),consumerRecord.offset()));
		}
