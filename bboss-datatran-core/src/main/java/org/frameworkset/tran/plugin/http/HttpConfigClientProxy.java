...
public class HttpConfigClientProxy {
	...
	public   <T> HttpResult<T>  putBodyForList(HttpInputDataTranPlugin httpInputDataTranPlugin, HttpResultParserContext httpResultParserContext, DynamicHeaderContext dynamicHeaderContext, Map params, final Class<T> resultType) throws HttpProxyRequestException {
		...
		List<T> datas =  HttpRequestProxy.putBody(  httpInputConfig.getSourceHttpPool(),  dsl,   httpInputConfig.getQueryUrl(), HttpProxyHelper.getHttpHeaders(httpInputConfig,dynamicHeaderContext), ContentType.APPLICATION_JSON, baseURLResponseHandler);
		httpResult.setDatas(datas);
		return httpResult;
	}

    public   <T> HttpResult<T>  putForList(HttpInputDataTranPlugin httpInputDataTranPlugin, HttpResultParserContext httpResultParserContext, DynamicHeaderContext dynamicHeaderContext, Map params, final Class<T> resultType) throws HttpProxyRequestException {
        final HttpResult<T> httpResult = new HttpResult<>();
        HttpInputConfig httpInputConfig = httpInputDataTranPlugin.getHttpInputConfig();
    ...
    public   <T> HttpResult<T>  sendBodyForList(HttpInputDataTranPlugin httpInputDataTranPlugin,
                                                HttpResultParserContext httpResultParserContext,
                                                DynamicHeaderContext dynamicHeaderContext,
                                                Map params, final Class<T> resultType) throws HttpProxyRequestException {
        final HttpResult<T> httpResult = new HttpResult<>();
        HttpInputConfig httpInputConfig = httpInputDataTranPlugin.getHttpInputConfig();
        String dsl = this.evalTemplate(httpInputConfig.getQueryDslName(),params);
        if(logger.isInfoEnabled() && httpInputConfig.isShowDsl()){
        ...
        List<T> datas =  HttpRequestProxy.sendBody(  httpInputConfig.getSourceHttpPool(),  dsl,
                httpInputConfig.getQueryUrl(), HttpProxyHelper.getHttpHeaders(httpInputConfig,dynamicHeaderContext), ContentType.APPLICATION_JSON,baseURLResponseHandler );
//		httpResult.setDatas(datas);
        return httpResult;
    }


    public   <T> HttpResult<T>  postForList(HttpInputDataTranPlugin httpInputDataTranPlugin, HttpResultParserContext httpResultParserContext, DynamicHeaderContext dynamicHeaderContext,Map params, final Class<T> resultType) throws HttpProxyRequestException {
        final HttpResult<T> httpResult = new HttpResult<>();
        HttpInputConfig httpInputConfig = httpInputDataTranPlugin.getHttpInputConfig();
...
//		httpResult.setDatas(datas);
	...
	public   <T> HttpResult<T>  httpGetforList(HttpInputDataTranPlugin httpInputDataTranPlugin, HttpResultParserContext httpResultParserContext, DynamicHeaderContext dynamicHeaderContext,Map params, final Class<T> resultType) throws HttpProxyRequestException {
		final HttpResult<T> httpResult = new HttpResult<>();
		HttpInputConfig httpInputConfig = httpInputDataTranPlugin.getHttpInputConfig();
