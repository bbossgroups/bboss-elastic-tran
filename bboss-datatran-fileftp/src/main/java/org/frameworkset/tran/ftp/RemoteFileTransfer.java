...
public class RemoteFileTransfer {
	 ...
	 * @param remoteFile
	 */
	public static List<?> ls(final FtpConfig ftpConfig, final String remoteFile){
		DefaultFtpContextImpl ftpContext = new DefaultFtpContextImpl(ftpConfig);
		if (ftpContext.getTransferProtocol() == FtpConfig.TRANSFER_PROTOCOL_FTP) {

			return FtpTransfer.ls(remoteFile,ftpContext);

		} else {
			return SFTPTransfer.ls(remoteFile,ftpContext);
		}
	}
