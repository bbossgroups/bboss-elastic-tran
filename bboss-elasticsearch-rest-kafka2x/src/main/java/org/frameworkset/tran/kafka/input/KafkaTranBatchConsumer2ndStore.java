	...
	private void deserializeData(ConsumerRecord<Object,Object> consumerRecord,List<Record> results){
		...
		if (value instanceof List) {
			...
			for (int i = 0; i < rs.size(); i++) {
				Object v = rs.get(i);
				if (v instanceof Map) {
					results.add(new KafkaMapRecord(consumerRecord.key(), (Map<String, Object>) v,consumerRecord.offset()));
				} else {
					results.add(new KafkaStringRecord(consumerRecord.key(), (String) v,consumerRecord.offset()));
				}
			}
			//return new KafkaMapRecord((ConsumerRecord<Object, List<Map<String, Object>>>) data);
		} else if (value instanceof Map) {
			results.add( new KafkaMapRecord(consumerRecord.key(), (Map<String, Object>) value,consumerRecord.offset()));
		} else if (value instanceof String) {
			results.add(new KafkaStringRecord(consumerRecord.key(), (String) value,consumerRecord.offset()));
		}
		else{
			if(logger.isWarnEnabled()){
				logger.warn("unknown value type:{}",value.getClass().getName());
			}
			results.add(new KafkaStringRecord(consumerRecord.key(), String.valueOf( value),consumerRecord.offset()));
		}
